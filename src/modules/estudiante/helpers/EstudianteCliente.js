...
const ingresarEstudianteAPI=(bodyEstudiante)=>{
    axios.post(`http://localhost:8081/API/v1.0/Matricula/estudiantes` , bodyEstudiante,{headers:head}).then(r=>r.data);
}

const actualizarEstudianteAPI=(bodyEstudiante,id)=>{
    axios.put(`http://localhost:8081/API/v1.0/Matricula/estudiantes/${id}` , bodyEstudiante,{headers:head}).then(r=>r.data);
}

const eliminarEstudianteAPI=(cedula)=>{
    axios.delete(`http://localhost:8081/API/v1.0/Matricula/estudiantes/${cedula}`,{headers:head}).then(r=>r.data);
}

const obtenerEstudianteAxios=async(cedula)=>{
   
    const data = axios.get(`http://localhost:8081/API/v1.0/Matricula/estudiantes/${cedula}`,{headers:head}).then(r=>r.data);
